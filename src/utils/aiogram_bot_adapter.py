        ...
        if isinstance(file_obj, (str, os.PathLike)):
            ...
            if os.path.exists(path):
                if local_upload and self._is_local_api():
                    # telegram-bot-api --local can read the file directly from
                    # the shared filesystem. A proper file URI is required;
                    # passing a bare /absolute/path is interpreted as a broken
                    # HTTP URL by the Bot API.
